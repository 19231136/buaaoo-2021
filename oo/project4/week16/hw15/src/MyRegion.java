...
public class MyRegion {
    ...
    private HashMap<String,List<String>> transitionid2stateid = new HashMap<>();
    ...
    public void addState(MyState myState) {
        if (myState.getType() == ElementType.UML_STATE) {
            ...
            states.add(myState);
        } else if (myState.getType() == ElementType.UML_PSEUDOSTATE) {
            states.add(myState);
            init = myState;
        }
    }
    ...
    public void addTransition(UmlTransition umlTransition,MyState source,MyState target) {
        ...
        stateId.add(source.getId());
        stateId.add(target.getId());
        transitionid2stateid.put(umlTransition.getId(),stateId);
        source.addSubsequentState(target);
        id2transition.put(umlTransition.getId(),umlTransition);
    }
    ...
    public void checkForUml008() throws UmlRule008Exception {
        if (init != null) {
            if (init.getTransitionSum() > 1) {
                throw new UmlRule008Exception();
            } else if (init.getTransitionSum() == 1) {
                List<MyState> states = init.getStates();
                MyState stateTo = null;
                for (MyState state : states) {
                    stateTo = state;
                }
                for (String id : transitionid2stateid.keySet()) {
                    List<String> list1 = transitionid2stateid.get(id);
                    if (list1.get(0).equals(init.getId())
                            && list1.get(1).equals(stateTo.getId())) {
                        if (transitionid2triggers.get(id) != null) {
                            throw new UmlRule008Exception();
                        }
                        if (id2transition.get(id).getGuard() != null) {
                            throw new UmlRule008Exception();
                        }
                    }
                }
