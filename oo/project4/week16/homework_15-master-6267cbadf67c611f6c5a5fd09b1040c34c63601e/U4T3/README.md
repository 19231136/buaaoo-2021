...
学生需要实现一个自己的`UmlGeneralInteraction`类，这个类必须继承接口`com.oocourse.uml3.interact.format.UmlGeneralInteraction`。
...
```bash
java -jar U4T3.jar list -s "R001.mdj"
```
...
```bash
java -jar U4T3.jar list -s "R001.mdj" -t UMLModel -n Model
```
...
```bash
java -jar U4T3.jar dump -s "R13.mdj" -n Model -t UMLModel
