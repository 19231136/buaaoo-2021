...
public class JvmHeap extends MyHeap<MyObject> {
    ...
    public void removeUnreferenced() {
        // TODO
        int i = 1;
        Object[] newElementData = new Object[getSize()];
        while (this.getSize() > 0) {
            MyObject myObject = (MyObject) this.getElementData()[1];
            if (myObject.getReferenced()) {//true
                newElementData[i++] = myObject;
            }
            this.removeFirst();
        }
        for (int j = 1;j < i;j++) {
            this.add((MyObject) newElementData[j]);
        }
    }
    ...
    public /*@pure@*/ MyObject getYoungestOne() {
        ...
        MyObject min = (MyObject) this.getElementData()[1];
        for (int i = 1; i < this.getSize();i++) {
            MyObject myObject = (MyObject) this.getElementData()[i];
            if (myObject.compareTo(min) < 0) {
                min = myObject;
            }
        }
        return min;
